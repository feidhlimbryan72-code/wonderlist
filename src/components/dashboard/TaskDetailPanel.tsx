...
export const TaskDetailPanel: React.FC<TaskDetailPanelProps> = ({ task, listId, onClose, readOnly = false }) => {
    ...
    <div className="w-full lg:w-96 bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 h-full flex flex-col text-slate-900 dark:text-white">
      ...
      <div className="p-4 border-b border-slate-200/20 dark:border-white/5 flex items-center justify-between">
        <button
          ...
          disabled={readOnly}
          className="flex items-center gap-2 hover:bg-slate-200/50 dark:hover:bg-white/5 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {task.is_completed ? (
            <>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6">
        ...
        <div className="space-y-1">
          <input
            ...
            disabled={readOnly}
            className={`w-full bg-transparent border-0 focus:ring-0 text-lg font-bold p-0 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none disabled:opacity-60 ${task.is_completed ? 'line-through text-slate-400 dark:text-slate-500' : ''}`}
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            disabled={readOnly}
            className="w-full px-3 py-2.5 bg-slate-100/60 dark:bg-slate-800/40 border border-slate-300/40 dark:border-white/5 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-slate-800 dark:text-white text-xs transition-all disabled:opacity-60"
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            disabled={readOnly}
            className="w-full px-3 py-2.5 bg-slate-100/60 dark:bg-slate-800/40 border border-slate-300/40 dark:border-white/5 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500/50 text-slate-800 dark:text-white text-xs transition-all cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
          >
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            rows={6}
            className="w-full px-3.5 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-slate-900 dark:text-white text-xs transition-all resize-none placeholder-slate-400 disabled:opacity-60"
          />
        </div>
